...
internal sealed class TestRequestAgent(TestAgentRequestType requestType, int unpairedRequestCount, int pairedRequestCount, string? id, string? name) : AIAgent
{
    ...
    private sealed class FunctionApprovalStrategy : IRequestResponseStrategy<InputRequestContent, InputResponseContent>
    {
        public InputResponseContent CreatePairedResponse(InputRequestContent request)
        {
            ...
            return new FunctionApprovalResponseContent(approvalRequest.RequestId, true, approvalRequest.FunctionCall);
        }
        ...
        public IEnumerable<(string, InputRequestContent)> CreateRequests(int count)
        {
            for (int i = 0; i < count; i++)
            {
                ...
                InputRequestContent request = new FunctionApprovalRequestContent(id, new(id, "TestFunction"));
        ...
        where TRequest : AIContent
    {
        return request switch
        {
            ...
            InputRequestContent userInputRequest => userInputRequest.RequestId,
